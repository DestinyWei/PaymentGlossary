#!/usr/bin/env python3
"""Import original article illustrations from the AllScale DOCX collection."""
...
def main() -> None:
    ...
    parser.add_argument("--content", type=Path, default=Path("articles/content.json"))
    ...
    for article in articles:
        ...
        hero_path = args.assets / f"{article['slug']}.webp"
        ...
        for number, section in enumerate(article_images["sections"], start=2):
            image_path = args.assets / f"{article['slug']}-{number:02d}.webp"
    ...
    print(f"Imported 80 illustrations across {len(articles)} articles")
